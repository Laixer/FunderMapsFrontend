  ...
  <div class="FormField GeoCoder" :class="fieldClasses">
    ...
    <div class="FormField__Wrapper">
      ...
      <div v-if="suggestions.length !== 0" class="GeoCoder__Suggestions">
        <div v-for="address in suggestions" :key="address.id" class="Suggestion" @click="handleSelect(address)">
...
import { FormField, SvgIcon, Address } from "@fundermaps/common";
    ...
    if (target.value.length >= 3) {
      try {
        ...
        const json = await fetch(
          `${process.env.VUE_APP_API_BASE_URL}/api/incident-portal/address-suggest?query=${target.value}&limit=5`
        ).then(res => {
